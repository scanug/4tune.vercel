/**
 * LUCKY LIAR - Animations & Feedback (Sezione 9.2)
 * Animazioni e feedback per bluff, sfide, cambio crediti
 */
 ...
 * Appare e scompare automaticamente
 */
export function FeedbackToast({ message, type, duration = 3000, onDismiss }) {
...
// ============================================
// BLUFF RESULT ANIMATION
// ============================================

/**
 * Animazione e feedback per risultato bluff
 */
export function BluffResultAnimation({ success, playerName, onComplete }) {
  useEffect(() => {
    const timeout = setTimeout(() => {
      onComplete?.();
 ...
 */
export function ChallengeAnimation({ claimerName, challengerName, onReady }) {
  useEffect(() => {
    const timeout = setTimeout(() => {
      onReady?.();
 ...
 */
export function CreditChangePopup({ amount, x, y, onComplete }) {
  useEffect(() => {
    const timeout = setTimeout(() => {
      onComplete?.();
 ...
 */
export function LoadingIndicator({ text = 'In caricamento...', size = 'md' }) {
  const sizes = {
 ...
 */
export function PulseAnimation({ children, color = COLORS.accent.primary }) {
  return (
    <div className="pulse-container" style={{ '--pulse-color': color }}>
      {children}
 ...
 */
export function FeedbackToastContainer({ feedbacks, onDismiss }) {
  return (
    <>
