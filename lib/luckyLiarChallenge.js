...
// Penalità standard
export const CHALLENGE_PENALTIES = {
  FAILED_CHALLENGE: 50,   // Chi sfida e perde
  SUCCESSFUL_BLUFF: 75,   // Chi è stato sfidato e la dichiarazione era vera (bonus)
  CAUGHT_BLUFF: 100,      // Chi è stato sfidato e stava bluffando (penalità doppia)
};
...
export function resolveChallenge(challenge, playerHands, wildcards = null, activateWildcardFor = null) {
  ...
  // Determina outcome, perdente e penalità
  let outcome, loserId, loserName, penalty, penaltyReason, explanation;
  ...
  if (isTrueClaim) {
    ...
    penalty = CHALLENGE_PENALTIES.FAILED_CHALLENGE;
    penaltyReason = 'Sfida fallita';
    explanation = `${challenge.claimerName} aveva ragione! C'erano davvero ${actualCount} ${challenge.claim.value}(i). ${challenge.challengerName} perde ${penalty} crediti.`;
  } else {
    // ✗ Dichiarazione falsa
    ...
    // Penalità escalate in base alla distanza dal bluff
    if (difference === 1) {
      penalty = 50; // Bluff minore
    } else if (difference <= 3) {
      penalty = CHALLENGE_PENALTIES.CAUGHT_BLUFF; // Bluff moderato
    } else {
      penalty = 150; // Bluff aggressivo
    }
    
    penaltyReason = `Bluff scoperto (dichiarato ${challenge.claim.quantity}, reali ${actualCount})`;
    explanation = `BLUFF SCOPERTO! ${challenge.claimerName} ha dichiarato ${challenge.claim.quantity} ma ce n'erano solo ${actualCount}. Perde ${penalty} crediti.`;
  }
  ...
  if (activateWildcardFor && wildcards) {
    ...
    if (validation.valid) {
      ...
      // Crea effetto wildcard
      wildcardEffect = createWildcardEffect(true, scenario, penalty);
...
/**
 * Calcola le penalità da applicare dopo una sfida
 * 
 * @param {ChallengeResult} result
 * @returns {Object} Mappa di penalità { playerId: { amount, reason } }
 */
export function calculatePenalties(result) {
  const penalties = {};
  penalties[result.loserId] = {
...
/**
 * Applica le penalità ai crediti dei giocatori
 * 
 * @param {Object} playerCredits - { playerId: number }
 * @param {Object} penalties - { playerId: { amount, reason } }
 * @returns {Object} Crediti aggiornati
 */
export function applyPenaltiesToCredits(playerCredits, penalties) {
  const updated = { ...playerCredits };
  for (const playerId in penalties) {
    const penalty = penalties[playerId];
    updated[playerId] = Math.max(0, (updated[playerId] || 0) - penalty.amount);
  }
  return updated;
}
