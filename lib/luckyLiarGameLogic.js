...
export const GAME_PHASES = {
  ...
  BETTING: 'betting',       // Fase scommesse
...
export const PENALTIES = {
  FAILED_CHALLENGE: 50,    // Crediti persi se sfida fallisce
  PASS_PENALTY: 10,        // Crediti persi per passare
};
 ...
 * @param {string[]} playerIds
 * @param {Object} players - { playerId: { name, avatar, credits } }
 * @param {number} roundNumber
 * @param {number} initialWager - Scommessa iniziale per giocatore
 * @param {string} wildcardMode - SINGLE o DOUBLE (default SINGLE)
 * @returns {RoundState}
 */
export function initializeRound(playerIds, players, roundNumber, initialWager = 50, wildcardMode = WILDCARD_MODES.SINGLE) {
  ...
  // Inizializza scommesse
  const playerWagers = {};
  for (const playerId of playerIds) {
    playerWagers[playerId] = initialWager;
  }
